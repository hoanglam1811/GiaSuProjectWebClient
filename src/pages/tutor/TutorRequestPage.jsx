...
import { calculateEndTime, formatDate, formatTimeString, isTimeDifferenceGreaterThan } from "../../services/utils";
import {
...
export default function TutorRequestsPage() {
  ...
  const handleApply = async (bookingId) => {
    try {
      ...
      for (let schedule of bookingToApply.schedules) {
        if (isOverlapping(existingSchedules, schedule)) {
          setSnackbarMessage(
            "Overlapping schedule. Please choose other requests",
          );
        ...
        for (let existingSchedule of existingSchedules) {
          if (
            ...
            (!isTimeDifferenceGreaterThan(
              calculateEndTime(schedule.startTime, schedule.duration),
      ...
      if (!hasActiveCredential) {
        ...
        setSnackbarMessage(
          "Your credential for this subject is not approved yet",
        );
        setSnackbarOpen(true);
        return;
      }
